...
// QuerySwimLane chains the current query on the "swim_lane" edge.
func (_q *TaskQuery) QuerySwimLane() *SwimLaneQuery {
	query := (&SwimLaneClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TaskQuery) QuerySprint() *SprintQuery {
...
func (_q *TaskQuery) QueryAttachments() *TaskAttachmentQuery {
...
func (_q *TaskQuery) FirstID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (_q *TaskQuery) FirstIDX(ctx context.Context) int64 {
...
func (_q *TaskQuery) OnlyID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (_q *TaskQuery) OnlyIDX(ctx context.Context) int64 {
...
func (_q *TaskQuery) IDs(ctx context.Context) (ids []int64, err error) {
...
func (_q *TaskQuery) IDsX(ctx context.Context) []int64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
// WithSwimLane tells the query-builder to eager-load the nodes that are connected to
// the "swim_lane" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TaskQuery) WithSwimLane(opts ...func(*SwimLaneQuery)) *TaskQuery {
	query := (&SwimLaneClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSwimLane = query
	return _q
}

// WithSprint tells the query-builder to eager-load the nodes that are connected to
// the "sprint" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TaskQuery) WithSprint(opts ...func(*SprintQuery)) *TaskQuery {
	query := (&SprintClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithAttachments(opts ...func(*TaskAttachmentQuery)) *TaskQuery {
	query := (&TaskAttachmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withSwimLane; query != nil {
		if err := _q.loadSwimLane(ctx, query, nodes, nil,
			func(n *Task, e *SwimLane) { n.Edges.SwimLane = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSprint; query != nil {
		if err := _q.loadSprint(ctx, query, nodes, nil,
			func(n *Task, e *Sprint) { n.Edges.Sprint = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withAttachments; query != nil {
		if err := _q.loadAttachments(ctx, query, nodes,
			func(n *Task) { n.Edges.Attachments = []*TaskAttachment{} },
			func(n *Task, e *TaskAttachment) { n.Edges.Attachments = append(n.Edges.Attachments, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TaskQuery) loadSwimLane(ctx context.Context, query *SwimLaneQuery, nodes []*Task, init func(*Task), assign func(*Task, *SwimLane)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Task)
	for i := range nodes {
		if nodes[i].SwimLaneID == nil {
			continue
		}
		fk := *nodes[i].SwimLaneID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "swim_lane_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskQuery) loadSprint(ctx context.Context, query *SprintQuery, nodes []*Task, init func(*Task), assign func(*Task, *Sprint)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Task)
	for i := range nodes {
		if nodes[i].SprintID == nil {
			continue
		}
		fk := *nodes[i].SprintID
		if _, ok := nodeids[fk]; !ok {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "sprint_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskQuery) loadComments(ctx context.Context, query *TaskCommentQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskComment)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(taskcomment.FieldTaskID)
	}
...
func (_q *TaskQuery) loadAttachments(ctx context.Context, query *TaskAttachmentQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskAttachment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(taskattachment.FieldTaskID)
	}
...
func (_q *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSwimLane != nil {
			_spec.Node.AddColumnOnce(task.FieldSwimLaneID)
		}
		if _q.withSprint != nil {
			_spec.Node.AddColumnOnce(task.FieldSprintID)
		}
