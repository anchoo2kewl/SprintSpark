...
// QuerySwimLanes chains the current query on the "swim_lanes" edge.
func (_q *ProjectQuery) QuerySwimLanes() *SwimLaneQuery {
...
func (_q *ProjectQuery) QueryAttachments() *TaskAttachmentQuery {
...
func (_q *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withSwimLanes:   _q.withSwimLanes.Clone(),
...
// WithSwimLanes tells the query-builder to eager-load the nodes that are connected to
// the "swim_lanes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ProjectQuery) WithSwimLanes(opts ...func(*SwimLaneQuery)) *ProjectQuery {
	query := (&SwimLaneClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSwimLanes = query
	return _q
}
...
func (_q *ProjectQuery) WithAttachments(opts ...func(*TaskAttachmentQuery)) *ProjectQuery {
	query := (&TaskAttachmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withSwimLanes; query != nil {
		if err := _q.loadSwimLanes(ctx, query, nodes,
			func(n *Project) { n.Edges.SwimLanes = []*SwimLane{} },
			func(n *Project, e *SwimLane) { n.Edges.SwimLanes = append(n.Edges.SwimLanes, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAttachments; query != nil {
		if err := _q.loadAttachments(ctx, query, nodes,
			func(n *Project) { n.Edges.Attachments = []*TaskAttachment{} },
			func(n *Project, e *TaskAttachment) { n.Edges.Attachments = append(n.Edges.Attachments, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProjectQuery) loadSwimLanes(ctx context.Context, query *SwimLaneQuery, nodes []*Project, init func(*Project), assign func(*Project, *SwimLane)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(swimlane.FieldProjectID)
	}
	query.Where(predicate.SwimLane(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.SwimLanesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ProjectQuery) loadAttachments(ctx context.Context, query *TaskAttachmentQuery, nodes []*Project, init func(*Project), assign func(*Project, *TaskAttachment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(taskattachment.FieldProjectID)
	}
...
func (_q *ProjectQuery) loadWikiPages(ctx context.Context, query *WikiPageQuery, nodes []*Project, init func(*Project), assign func(*Project, *WikiPage)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(wikipage.FieldProjectID)
	}
...
func (_q *ProjectQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withOwner != nil {
			_spec.Node.AddColumnOnce(project.FieldOwnerID)
		}
		if _q.withTeam != nil {
